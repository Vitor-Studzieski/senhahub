...
test("nova chamada abre destaque exclusivo por sete segundos e tenta emitir aviso sonoro", () => {
  ...
  const page = fs.readFileSync(path.join(root, "app/tv/acougue/page.jsx"), "utf8");
...
test("vídeos enviados para a TV são normalizados para H.264 e AAC", () => {
  ...
  const runtime = fs.readFileSync(path.join(root, "server/integrations/supabase-runtime.js"), "utf8");
  const client = fs.readFileSync(path.join(root, "public/marketing-tv.js"), "utf8");
  const tvClient = fs.readFileSync(path.join(root, "public/tv-acougue.js"), "utf8");
  ...
  assert.match(runtime, /createTvMediaStreamToken/);
  assert.match(runtime, /verifyTvMediaStreamToken/);
  assert.match(runtime, /searchParams\.get\("token"\)/);
  assert.match(runtime, /streamToken/);
  assert.match(runtime, /accept-ranges/);
  assert.match(runtime, /\/api\/tv\/media\/\$\{row\.id\}\/stream\?token=/);
  ...
  assert.match(tvClient, /playlistSourceKey/);
  assert.match(tvClient, /short-lived stream URLs/);
