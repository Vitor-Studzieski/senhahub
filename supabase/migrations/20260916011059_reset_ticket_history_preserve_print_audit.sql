-- Let manager history resets remove only terminal tickets whose print retries
-- are definitively exhausted, while retaining the associated print audit rows.
create or replace function public.protect_unresolved_print_ticket() returns trigger
language plpgsql
set search_path = ''
as $$
begin
  if exists (
    ...
    where (j.ticket_id = old.id or j.payload->'ticketIds' @> to_jsonb(array[old.id::text]))
...
begin
  ...
  -- Pending, active, or uncertain receipts stay protected. Exhausted failed
  -- receipts are retained as audit rows, with ticket_id detached by its FK.
  for v_ticket_id in
    select t.id
    from public.tickets t
