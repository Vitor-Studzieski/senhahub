-- Allow managers to clear closed ticket history after test runs without
-- disturbing active queues or the ticket numbering counters.
...
begin
  ...
  -- Lock the terminal rows selected for deletion so active tickets that finish
  -- after this reset begins remain available as new operational history.
  for v_ticket_id in
    select t.id
  ...
  insert into public.events (
    ...
    p_actor_id,
    null,
    jsonb_build_object('deleted_tickets', v_deleted_tickets, 'deleted_ratings', v_deleted_ratings),
    now()
  );
