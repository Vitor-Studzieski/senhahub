...
export async function POST(request) {
  ...
  if (!await consumeLocalPushRateLimit(session.user.id, "subscribe", 10, 60 * 60, request.headers.get("user-agent") || "local")) {
    return Response.json({ error: "Muitas tentativas de inscricao. Aguarde e tente novamente." }, { status: 429 });
  }
  let body;
  try { body = await request.json(); } catch { return Response.json({ error: "JSON invalido." }, { status: 400 }); }
  try {
    const result = await subscribeLocalPush(session.user.id, body, request.headers.get("user-agent") || "");
    return Response.json({ ok: true, ...result }, { status: 201 });
  } catch (error) {
    const status = /ainda nao foram configuradas/i.test(error.message) ? 503 : 400;
    return Response.json({ error: error.message || "Nao foi possivel cadastrar este dispositivo." }, { status });
  }
}
